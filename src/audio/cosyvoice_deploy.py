...
def check_cosyvoice_installed() -> bool:
    """检查 CosyVoice 是否已安装"""
    return COSYVOICE_DIR.exists() and (COSYVOICE_DIR / "webui.py").exists()
...
def start_cosyvoice(port: int = 5000) -> subprocess.Popen | None:
    """启动 CosyVoice 服务
    ...
    # CosyVoice 的 webui.py 默认启动 HTTP 服务
    webui_path = COSYVOICE_DIR / "webui.py"
    api_path = COSYVOICE_DIR / "api.py"

    # 优先使用 api.py（纯 API 模式，无 Gradio UI）
    entry_script = str(api_path if api_path.exists() else webui_path)

    logger.info(f"正在启动 CosyVoice 服务: {host}:{port}")
    ...
    try:
        ...
        if proc.poll() is not None:
            ...
            logger.error(f"CosyVoice 启动失败: {stderr}")
            return None
...
def check_cosyvoice_health(host: str = "127.0.0.1", port: int = 5000) -> bool:
    """检查 CosyVoice 服务健康状态
